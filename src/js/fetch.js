...
const workPerformerTypes = {'Chamber' : ['Ensemble'], 'Orchestral':['Conductor', 'Orchestra'], 'Keyboard':['Piano'], 'Stage':['Conductor', 'Orchestra'], 'Concerto' : ['Solist', 'Conductor', 'Orchestra']};
...
function getGenres(){
    ...
    request.onreadystatechange = function () {
        if (request.readyState==4 && this.status == 200) {
            ...
            let HTMLString = `<option value="Select genre">Select genre...</option>`;
                ...
                for (let i = 0; i < albums.length; i++){
                    if (values[i] != -1 && albums[i]['songs'].length > trackNumbers[i][values[i]]) {
                        ...
                        getRoles(albums[i]['songs'][trackNumbers[i][values[i]] - 1]['artistName']).then((roles) => {
                            ...
                            // if (!(roles['Orchestra'] in orchestras)){
                            //     let size = orchestras.length;
                            //     document.getElementById('orchestras').innerHTML += `<input class="form-check-input" type="checkbox" value="" id="defaultCheck${size">
